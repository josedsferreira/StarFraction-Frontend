...
export async function getUserPlanetsWithDetailsClient(token: string, userId: string) {
	try {
		const response = await axios.get(`${API_BASE_URL}/users/userplanets/withdetails/${userId}`, {
			headers: {
				'Authorization': `Bearer ${token}`
			}
		});
...
export async function getUpgradeStatusClient(token: string, planetId: number) {
	try {
		const response = await axios.get(`${API_BASE_URL}/planets/${planetId}/upgradeStatus`, {
			headers: {
				'Authorization': `Bearer ${token}`
			}
		});
...
export async function upgradeBuildingClient(token: string, planetId: number, buildingName: string) {
	try {
		const response = await axios.post(`${API_BASE_URL}/planets/${planetId}/upgradeBuilding/${buildingName}`, {
			headers: {
				'Authorization': `Bearer ${token}`
			}
		});
